...
void join_network(const uint8_t max_attempts)
{
    // int32_t j_attempts = 0;
    int32_t ret = mDot::MDOT_ERROR;

    // attempt to join the network
    for (int8_t j_attempts = 0; j_attempts < max_attempts; j_attempts++)
    {
        if (ret == mDot::MDOT_OK)
        {
            logInfo("Joined to network");
            return;
        }
    ...
    logError("failed to join network after %u attempts", max_attempts);
}
